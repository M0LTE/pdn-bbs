...
/// <summary>
/// Resolves the X.509 certificate for the optional implicit-TLS IMAP listener: loads an
...
/// across restarts. Ported from the node's <c>TlsCertificateProvider</c>: RSA-2048 / SHA-256, the
/// serverAuth EKU, and a SAN of <c>localhost</c> + the machine name + the bind address.
/// </summary>
/// <remarks>
...
/// </remarks>
public static partial class ImapTlsCertificate
{
    // serverAuth EKU OID — a TLS client expects the server cert to assert it.
    private const string ServerAuthOid = "1.3.6.1.5.5.7.3.1";

    /// <summary>
    /// Resolves the IMAP TLS certificate, or null when none can be produced. <paramref name="certificatePath"/>
    ...
    [LoggerMessage(Level = LogLevel.Error, Message = "IMAP TLS: configured certificatePath '{Path}' does not exist; TLS listener not started.")]
    private static partial void LogSuppliedCertMissing(ILogger logger, string path);

    [LoggerMessage(Level = LogLevel.Error, Message = "IMAP TLS: no certificatePath and generateSelfSigned is false; TLS listener not started.")]
    private static partial void LogNoCertConfigured(ILogger logger);

    [LoggerMessage(Level = LogLevel.Information, Message = "IMAP TLS: persisted self-signed cert at '{Path}' expired; regenerating.")]
    private static partial void LogSelfSignedExpired(ILogger logger, string path);

    [LoggerMessage(Level = LogLevel.Information, Message = "IMAP TLS: generated a self-signed certificate at '{Path}' (valid until {NotAfter:u}); clients warn until it is trusted.")]
    private static partial void LogGenerated(ILogger logger, string path, DateTime notAfter);

    [LoggerMessage(Level = LogLevel.Error, Message = "IMAP TLS: failed to resolve a certificate; TLS listener not started.")]
