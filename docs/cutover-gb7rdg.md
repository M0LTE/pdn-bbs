...
| `connect-test` | The per-partner **bring-up**: every partner is imported disabled, so here you `Test connect` each (moving **no mail**), tighten its `EXPECT=` strings from the real prompt, then **Enable** the good ones in the BBS sysop UI (`POST /apps/bbs/forwarding/{test-connect,partner/enable}`). Direct-BBS partners need only connect; the multi-hop ones (GB7LOX `… bbs`, GB7CIP `C GB7WEM-7`→`C uhf gb7cip`) need real expects. A partner left disabled simply won't forward. Run after `network`, before `golive`. | n/a |
