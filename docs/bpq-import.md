...
- **Housekeeping retention MUST be set in `bbs.yaml` to match BPQ — or live mail is lost on first boot.** The importer rebuilds the mailbox, but message *lifetimes* are pdn-bbs **config**, not bbs.db data. pdn-bbs's built-in defaults are short (bulletins ~7 days, personals ~30 days); GB7RDG's `linmail.cfg` keeps everything `MaxAge=365`. With the defaults, the first housekeeping sweep purges everything older than them — in the GB7RDG rehearsal that was **49 of 192 messages, including unread personals** (mail loss). Set the `housekeeping:` block (all `*Days` to 365, `bidLifetimeDays: 60`) in the node's BBS `bbs.yaml` BEFORE first start. Verified: with it set, the only purge is the already-killed (`K`) messages — `killed 0 by age`.
- **Route-on-load re-evaluates distribution (BID-safe, but not BPQ's exact queue).** On every start pdn-bbs's `RouteStartupBacklog` sweeps the whole store and queues each message to every partner whose distribution (`AtCalls`/recipient) matches and that lacks a leg. On a migrated mailbox this **adds** queued legs BPQ didn't have (pdn's distribution logic ≠ BPQ's historical bits) — ~+135 in the rehearsal. This does NOT cause duplicate *delivery*: the pre-marked **sent** legs are preserved exactly (so BPQ-sent mail is never re-sent), the BID dedup store is intact (a partner rejects a BID it already has), and source-reflection is guarded. The effect is extra forward *proposals* for already-widespread bulletins (BID-rejected) plus genuinely-pending mail (correct to forward). If exact-BPQ-queue fidelity is wanted instead, seed the routing watermark to the high-water mark so migrated messages are not re-routed (not currently implemented — it has a crash-recovery trade-off).
